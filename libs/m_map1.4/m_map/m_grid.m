...
glinestyle=':';
glinewidth=get(AXE,'linewidth');
gbox='on'; 
gfontsize=get(AXE,'fontsize');
gfontname=get(AXE,'fontname');
gxaxisloc=get(AXE,'xaxislocation'); 
gyaxisloc=get(AXE,'yaxislocation');
gtickdir=get(AXE,'tickdir'); 
gticklen=get(AXE,'ticklength'); gticklen=gticklen(1); 
...
if MAP_PROJECTION.IsOctave
  ...
  % A bug in Octave 3.8.1 makes set(AXE, 'children', -) insert the new
  ...
  set(0, 'ShowHiddenHandles', show);
   ...
   if ~MAP_PROJECTION.newgraphics
	 ...
	 set(0, 'ShowHiddenHandles', show);
       ...
       axes(AXE)
     XY = patch(AXE,'xdata',X(:),'ydata',Y(:),'zdata',-MAP_PROJECTION.LARGVAL*ones(size(X(:))),'facecolor',[0.4941    0.7529    0.9333],...
             'edgecolor','k','linestyle','none','tag','m_grid_color');
%     patch('xdata',X(:),'ydata',Y(:),'facecolor',gbackcolor,...
%             'edgecolor','k','linestyle','none','tag','m_grid_color');
	 set(AXE,'sortmethod','childorder');
     set(AXE,'children',flipud(get(AXE,'children')))
%      uistack(findobj(AXE,'tag','m_grid_color'),'bottom');
...
if ~isempty(xtick)
 ...
 [n,m]=size(X);
 line(AXE,reshape([X;NaN+ones(1,m)],(n+1)*m,1),reshape([Y;NaN+ones(1,m)],(n+1)*m,1),...
 ...
 if drawticks
   [n,m]=size(ltx);
   line(AXE,reshape([ltx;NaN+ones(1,m)],(n+1)*m,1),reshape([lty;NaN+ones(1,m)],(n+1)*m,1),...
        'linestyle','-','color',gcolor,'linewidth',glinewidth,'tag','m_grid_xticks-lower','clipping','off');
   line(AXE,reshape([utx;NaN+ones(1,m)],(n+1)*m,1),reshape([uty;NaN+ones(1,m)],(n+1)*m,1),...
 ...
 for k=ik
   ...
   text(AXE,xx(k),yy(k),labs{k},'horizontalalignment',horizk,'verticalalignment',vertk, ...
...
if strcmp(gbox,'on')
  [X,Y]=feval(MAP_PROJECTION.routine,'box');
  line(AXE,X(:),Y(:),'linestyle','-','linewidth',glinewidth,'color',gcolor,'tag','m_grid_box','clipping','off');
end
