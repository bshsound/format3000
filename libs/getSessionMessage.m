function SM = getSessionMessage(id,User,Password)
%% 1. Create Token
...
url = ['https://underwaternoise.ices.dk/continuous/api/getScreeningSessionMessages/' num2str(id) '/'];
options = weboptions('KeyName','Authorization','KeyValue',['Bearer ' jwt]);
SM = webread(url,options);
end